...
//calculate lane id from d value
int lane_id_from_d(double d)
{
    return (int)d%4;
}
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // returns [id, x, y, vx, vy, s, d], has type vector<vector<double>>
            ...
            for(int i = 0; i<sensor_fusion.size();i++)
            {
                ...
                {
                    // this car is in ego vehicle lane
                    ...
                    //check s value creater than mine and s gap
                    if((check_car_s >car_s) && ((check_car_s-check_car_s)<30))
                    {
                        // do some logic here, lower reference velocity so we dont
                        //crash into other car in front of use, or flag to try to change lane
                        too_close = true;
                        if(lane >0)
                        {
                            lane = 0;
            ...
            // define the actual (x,y) points we will us efor the planner
          	...
          	for(int i = 0; i<prev_size;i++)
            {
          	...
          	// calculate how to break up spline points so that we travel at desired ference velocity
